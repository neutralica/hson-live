...
import { CANONICAL_SCHEMA_EVALUATOR_LIMITS } from "./graph.js";
...
type EvaluationState = {
  steps: number;
  limits: Required<CanonicalEvaluationLimits>;
  unionWork: number;
  exhausted: boolean;
};
...
function projected(graph: VerifiedCanonicalSchemaGraph, ref: number, value: ProjectedCandidate, path: LivePath, state: EvaluationState, depth = 0): CanonicalGraphEvaluation {
  if (depth > state.limits.maxDepth || !step(state)) return resource(ref, path);
  const node = graph.nodes[ref];
  ...
  if (node.kind === "projected-array") {
    ...
    return merge(value.map((item, index) => projected(graph, node.item as number, item, [...path, index], state, depth + 1)), state);
  }
  if (node.kind === "projected-tuple") {
    if (!Array.isArray(value)) return mismatch(graph, ref, path, projected_type(value));
    const results = node.items.map((item, index) => projected(graph, item, index < value.length ? value[index] as OrderedProjectedValue : MISSING, [...path, index], state, depth + 1));
  ...
  if (node.kind === "projected-object") {
    ...
    const results = node.properties.map(([key, child]) => projected(graph, child, values.has(key) ? values.get(key) as OrderedProjectedValue : MISSING, [...path, key], state, depth + 1));
  ...
  if (node.kind === "projected-record") {
    if (!is_ordered_projected_object(value)) return mismatch(graph, ref, path, projected_type(value));
    return merge(value.entries.map(([key, child]) => projected(graph, node.value, child, [...path, key], state, depth + 1)), state);
  }
...
function document_item(graph: VerifiedCanonicalSchemaGraph, ref: number, value: HsonNode, path: readonly number[], state: EvaluationState, depth = 0): CanonicalGraphEvaluation {
  ...
  if (node?.kind === "document-item-union") {
    if (!add_union_work(state, node.choices.length)) return resource(ref, path);
    const branches = node.choices.map((choice) => document_item(graph, choice, value, path, state, depth + 1));
    if (branches.some((branch) => branch.ok)) return valid();
  ...
  const attrsResult = node.attrs === undefined ? valid() : document_attrs(graph, node.attrs, value.$_attrs ?? {}, path, state, depth + 1);
  ...
  const contentResult = document_content(graph, node.content, children, path, state, depth + 1);
  return merge([attrsResult, contentResult], state);
}

function document_attrs(graph: VerifiedCanonicalSchemaGraph, ref: number, input: unknown, path: readonly number[], state: EvaluationState, depth = 0): CanonicalGraphEvaluation {
  if (depth > state.limits.maxDepth || !step(state)) return resource(ref, path);
  const node = graph.nodes[ref];
  ...
  for (const property of node.properties) {
    ...
    const result = admitted === undefined
      ...
      : projected(graph, property.value, admit_attr(admitted), [], state, depth + 1);
  ...
  if (issues.length > state.limits.maxIssues) return resource(ref, path);
...
function document_content(graph: VerifiedCanonicalSchemaGraph, ref: number, children: readonly HsonNode[], path: readonly number[], state: EvaluationState, depth = 0): CanonicalGraphEvaluation {
  ...
  if (node?.kind === "document-repeat") {
    ...
    return merge(children.map((child, index) => document_item(graph, node.item, child, [...path, index], state, depth + 1)), state);
  }
  ...
  return merge(node.items.map((item, index) => document_item(graph, item, children[index] as HsonNode, [...path, index], state, depth + 1)), state);
}
...
function merge(results: readonly CanonicalGraphEvaluation[], state: EvaluationState): CanonicalGraphEvaluation { const issues = results.flatMap((result) => result.issues); if (issues.length > state.limits.maxIssues) return resource(issues[0]?.schemaNode ?? 0, issues[0]?.path ?? []); return issues.length === 0 ? valid() : invalid(issues); }
function step(state: EvaluationState): boolean { state.steps += 1; if (state.steps > state.limits.maxSteps) { state.exhausted = true; return false; } return true; }
function add_union_work(state: EvaluationState, amount: number): boolean { state.unionWork += amount; if (state.unionWork > state.limits.maxUnionWork) { state.exhausted = true; return false; } return true; }
...
function resolved_limits(limits?: CanonicalEvaluationLimits): Required<CanonicalEvaluationLimits> {
  return Object.freeze({
    maxSteps: limits?.maxSteps ?? CANONICAL_SCHEMA_EVALUATOR_LIMITS.maxSteps,
    maxIssues: limits?.maxIssues ?? CANONICAL_SCHEMA_EVALUATOR_LIMITS.maxIssues,
    maxDepth: limits?.maxDepth ?? CANONICAL_SCHEMA_EVALUATOR_LIMITS.maxDepth,
    maxUnionWork: limits?.maxUnionWork ?? CANONICAL_SCHEMA_EVALUATOR_LIMITS.maxUnionWork,
    maxContentItems: limits?.maxContentItems ?? CANONICAL_SCHEMA_EVALUATOR_LIMITS.maxContentItems,
  });
}
