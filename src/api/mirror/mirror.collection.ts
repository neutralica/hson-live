...
import type {
  ...
  CollectionMirrorChange,
  CollectionMirrorDiagnostics,
  CollectionMirrorItemContext,
  CollectionMirrorItemUpdate,
  CollectionMirrorKey,
  CollectionMirrorListener,
  CollectionMirrorMappingSummary,
  CollectionMirrorRenderResult,
  CollectionMirrorSnapshot,
  CollectionMirrorStatus,
} from "../../types/mirror.types.js";
import {
  COLLECTION_MIRROR_BRANCH_ATTACHED_ERROR_CODE,
  COLLECTION_MIRROR_DISPOSED_ERROR_CODE,
  COLLECTION_MIRROR_DUPLICATE_KEY_ERROR_CODE,
  COLLECTION_MIRROR_HOST_NOT_EMPTY_ERROR_CODE,
  COLLECTION_MIRROR_INVALID_BRANCH_ERROR_CODE,
  COLLECTION_MIRROR_INVALID_SOURCE_ERROR_CODE,
  COLLECTION_MIRROR_MAPPING_CONFLICT_ERROR_CODE,
  COLLECTION_MIRROR_MISSING_IDENTITY_ERROR_CODE,
  COLLECTION_MIRROR_RENDERER_CREATE_ERROR_CODE,
  COLLECTION_MIRROR_RENDERER_UPDATE_ERROR_CODE,
  COLLECTION_MIRROR_SOURCE_REPLACEMENT_ERROR_CODE,
  CollectionMirrorError,
} from "./mirror.collection.error.js";
...
type CollectionMirrorRecord<TItem extends JsonValue> = {
  ...
  tree: LiveTree;
  viewQuid: string;
  update: CollectionMirrorItemUpdate<TItem> | undefined;
  owner: RendererOwner;
  disposed: boolean;
};
...
export function reflect_collection<TItem extends JsonValue>(
  ...
  return new CollectionMirroror(options).publicHandle();
}

class CollectionMirroror<TItem extends JsonValue> {
  private source: LiveMapPathHandle<readonly TItem[]>;
  private readonly reflectHost: LiveTree;
  private readonly selectKey: CollectionMirrorOptions<TItem>["key"];
  private readonly render: CollectionMirrorOptions<TItem>["render"];
  private records: CollectionMirrorRecord<TItem>[] = [];
  private recordsByKey = new Map<CollectionMirrorKey, CollectionMirrorRecord<TItem>>();
  private sourceOff: (() => void) | undefined;
  private readonly listeners = new Set<CollectionMirrorListener>();
  private currentStatus: CollectionMirrorStatus = "initializing";
  private currentFailure: CollectionMirrorError | undefined;
  private firstFailure: CollectionMirrorError | undefined;
  private replacementFailure: CollectionMirrorError | undefined;
  ...
  public constructor(options: CollectionMirrorOptions<TItem>) {
    ...
    if (this.reflectHost.isDisposed) {
      throw new CollectionMirrorError(
        COLLECTION_MIRROR_INVALID_BRANCH_ERROR_CODE,
    ...
    try {
      ...
      throw asCollectionMirrorError(error, COLLECTION_MIRROR_INVALID_SOURCE_ERROR_CODE, "Initial keyed projection failed.");
    }
  }

  public publicHandle(): CollectionMirror<TItem> {
    const self = this;
  ...
  private onCommit(event: LiveMapFeedEvent): void {
    ...
    try {
      ...
      const failure = asCollectionMirrorError(
        error,
        COLLECTION_MIRROR_RENDERER_UPDATE_ERROR_CODE,
  ...
  private update(
    ...
    const staged: CollectionMirrorRecord<TItem>[] = [];
    const nextRecords: CollectionMirrorRecord<TItem>[] = [];
    ...
    this.currentStatus = "updating";
    const change: CollectionMirrorChange = Object.freeze({
      kind,
    ...
    try {
      ...
      for (let ordinal = 0; ordinal < nextRecords.length; ordinal += 1) {
        ...
        if (stagedSet.has(record)) {
          const batch: CollectionMirrorRecord<TItem>[] = [];
          let end = ordinal;
  ...
  private createRecord(item: DesiredItem<TItem>, usedTrees: Set<object>): CollectionMirrorRecord<TItem> {
    const owner = makeRendererOwner();
    const context = makeItemContext(item, owner);
    let result: CollectionMirrorRenderResult<TItem>;
    try {
    ...
    try {
      ...
      const record: CollectionMirrorRecord<TItem> = {
        key: item.key,
  ...
  private updateRecord(
    record: CollectionMirrorRecord<TItem>,
    source: LiveMapPathHandle<TItem>,
  ...
  private readCollection(source: LiveMapPathHandle<readonly TItem[]>): readonly DesiredItem<TItem>[] {
    ...
    try {
      ...
      throw asCollectionMirrorError(error, COLLECTION_MIRROR_INVALID_SOURCE_ERROR_CODE, "Keyed projection source could not be read.");
    }
  ...
  private assertUniqueKeys(keys: readonly CollectionMirrorKey[]): void {
    const seen = new Set<CollectionMirrorKey>();
  ...
  private replaceSource(source: LiveMapPathHandle<readonly TItem[]>): void {
    ...
    try {
      ...
      const reflectFailure = asCollectionMirrorError(
        error,
        COLLECTION_MIRROR_SOURCE_REPLACEMENT_ERROR_CODE,
        "Live projection source replacement failed.",
      );
      const failure = new CollectionMirrorError(
        COLLECTION_MIRROR_SOURCE_REPLACEMENT_ERROR_CODE,
  ...
  private synchronize(): void {
    ...
    try {
      ...
      const failure = asCollectionMirrorError(error, COLLECTION_MIRROR_RENDERER_UPDATE_ERROR_CODE, "Projection resynchronization failed.");
      this.fail(failure);
      this.notify();
      throw failure;
    }
  ...
  private subscribe(listener: CollectionMirrorListener): () => void {
  ...
  private snapshot(): CollectionMirrorSnapshot {
    return Object.freeze({
  ...
  private diagnostics(): CollectionMirrorDiagnostics {
    return Object.freeze({
      status: this.currentStatus,
      sourceRevisionLastApplied: this.lastAppliedRevision,
  ...
  private debugMappings(): readonly CollectionMirrorMappingSummary[] {
  ...
  private fail(error: CollectionMirrorError): void {
    this.firstFailure ??= error;
    this.currentFailure ??= error;
    this.currentStatus = "failed";
  }

  private removeRecord(record: CollectionMirrorRecord<TItem>): void {
  ...
  private disposeRecords(records: readonly CollectionMirrorRecord<TItem>[]): void {
...
function normalizeRenderResult<TItem extends JsonValue>(
  result: CollectionMirrorRenderResult<TItem>,
): Readonly<{
  tree: LiveTree;
  update: CollectionMirrorItemUpdate<TItem> | undefined;
...
function asCollectionMirrorError(
  error: unknown,
  fallbackCode: ConstructorParameters<typeof CollectionMirrorError>[0],
  fallbackMessage: string,
): CollectionMirrorError {
  return error instanceof CollectionMirrorError
    ? error
    : new CollectionMirrorError(fallbackCode, fallbackMessage, error);
}

function formatKey(key: CollectionMirrorKey): string {
