...
import {
  ...
  DOCUMENT_MIRROR_DISPOSED_ERROR_CODE,
  DOCUMENT_MIRROR_DELEGATION_TARGET_INVALID_ERROR_CODE,
  DOCUMENT_MIRROR_DELEGATION_UNSUPPORTED_ERROR_CODE,
  DOCUMENT_MIRROR_DOM_MAPPING_MISMATCH_ERROR_CODE,
  DOCUMENT_MIRROR_NODE_KIND_MISMATCH_ERROR_CODE,
  DOCUMENT_MIRROR_QUID_COLLISION_ERROR_CODE,
  DOCUMENT_MIRROR_UPDATE_FAILED_ERROR_CODE,
  DOCUMENT_MIRROR_QUID_MISMATCH_ERROR_CODE,
  DOCUMENT_MIRROR_REVISION_GAP_ERROR_CODE,
  DOCUMENT_MIRROR_ROOT_REPLACEMENT_FAILED_ERROR_CODE,
  DOCUMENT_MIRROR_SNAPSHOT_REVISION_MISMATCH_ERROR_CODE,
  DOCUMENT_MIRROR_TARGET_MISSING_ERROR_CODE,
  DOCUMENT_MIRROR_UNSUPPORTED_OPERATION_ERROR_CODE,
  DocumentMirrorError,
} from "./mirror.document.error.js";
import {
  apply_document_structural_transaction,
  plan_document_structural_transaction,
} from "./mirror.document.structure.js";
import {
  document_root_from_root,
  plan_document_root_convergence,
  type DocumentRootMaterial,
} from "./mirror.document.root.js";
import {
  bind_graph_runtime,
...
function reflect_document_binding_in_runtime(
  ...
  try {
    ...
    throw as_binding_error(cause, DOCUMENT_MIRROR_UPDATE_FAILED_ERROR_CODE, "Initial document binding capture failed.");
  }
  ...
  try {
    ...
    ACTIVE_DOCUMENT_BINDINGS.delete(map);
    throw as_binding_error(cause, DOCUMENT_MIRROR_UPDATE_FAILED_ERROR_CODE, borrowed
        ...
        pending = pending.flatMap((entry) => {
          ...
          return transformed.kind === "retired"
            ...
            : transformed.kind === "invalid"
              ? (() => { throw new DocumentMirrorError(DOCUMENT_MIRROR_UPDATE_FAILED_ERROR_CODE, transformed.reason); })()
  ...
  const on_observation = (observation: LiveMapCommitObservation): void => {
    ...
    try {
      ...
      fail(as_binding_error(cause, DOCUMENT_MIRROR_UPDATE_FAILED_ERROR_CODE, "Document attribute projection failed."));
    }
  };
  ...
  try {
    ...
    ACTIVE_DOCUMENT_BINDINGS.delete(map);
    throw as_binding_error(cause, DOCUMENT_MIRROR_UPDATE_FAILED_ERROR_CODE, "Document binding initialization failed.");
  }
