...
import { make_livemap_client_mirror_from_snapshot_internal, make_livemap_mirror_from_semantic_checkpoint_internal } from "../livemap/livemap.libraries.js";
...
type AnyCheckpoint = LocusHostedAggregatePersistedCheckpoint | LocusHostedAggregatePersistedManifest;
...
export function active_checkpoint_id(checkpoint: AnyCheckpoint): string {
  return checkpoint.format === "hson-locus-durable-aggregate-checkpoint-v2"
    ? checkpoint.checkpointId
    : `v1:${checkpoint.incarnationId}:${checkpoint.rev}:${checkpoint.registryDigest}`;
}
...
async function validate_hosted_aggregate_state(
  ...
  try {
    ...
    const checkpoint = checkpointValue.format === "hson-locus-durable-aggregate-checkpoint-v2"
      ? assert_checkpoint_manifest(checkpointValue, requestedLogicalMapId)
      : assert_checkpoint_fence(checkpointValue, requestedLogicalMapId);
    if (!Array.isArray(state.commits)) throw invalid_state();

    const map = checkpoint.format === "hson-locus-durable-aggregate-checkpoint-v2"
      ? await restore_manifest(checkpoint, adapter)
      : make_livemap_client_mirror_from_snapshot_internal(durable_aggregate_snapshot_as_client(assert_snapshot_fence(checkpoint)));
    const aggregate = internal_livemap_aggregate_authority(map);
    if (checkpoint.format === "hson-locus-durable-aggregate-checkpoint-v1"
      && !exact_representation_equal(hosted_checkpoint(aggregate.captureHosted()).snapshot, checkpoint.snapshot)) throw invalid_state();
