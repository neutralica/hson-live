...
const HSON_SOURCE_TYPE_REQUIRED = "HSON_SOURCE_TYPE_REQUIRED";
const HSON_TEMPLATE_SUBSTITUTION_TYPE_REQUIRED =
  "HSON_TEMPLATE_SUBSTITUTION_TYPE_REQUIRED";

function admitHsonSource(source: string): HsonString {
...
function encodeTemplatePrimitive(value: HsonTemplatePrimitive): string {
  if (typeof value === "number") {
    // Numeric policy belongs to hsonNumber; the shared scalar serializer owns
    // canonical source spelling, including preservation of negative zero.
    return serialize_primitive_hson(hsonNumber(value));
  }
...
/**
 * Admit authored HSON source or author HSON with typed primitive substitutions.
 *
...
export function admit_hson(source: string): HsonString;
export function admit_hson(
  source: string | TemplateStringsArray,
  ...substitutions: readonly HsonTemplatePrimitive[]
): HsonString {
  if (typeof source === "string") return admitHsonSource(source);
  if (!isTemplateStringsArray(source)) {
    _throw_transform_err(
      `hson() requires authored HSON source text; received ${typeof source}`,
      "hson",
      undefined,
      undefined,
      { code: HSON_SOURCE_TYPE_REQUIRED, stage: "source-admission" },
    );
  }
