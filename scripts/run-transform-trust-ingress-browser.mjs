...
const fixtureRoot = await mkdtemp(join(temporaryRoot, "transform-trust-ingress-"));
...
function chromeExecutable() {
  ...
  throw new Error("Transform trust-ingress browser acceptance requires Chrome or Chromium.");
}
...
try {
  await copyFile(
    resolve(repositoryRoot, "tests/browser/transform-trust-ingress.acceptance.html"),
    join(fixtureRoot, "index.html"),
  );
  const bundle = spawnSync(resolve(repositoryRoot, "node_modules/.bin/esbuild"), [
    resolve(repositoryRoot, "tests/browser/transform-trust-ingress.entry.mjs"),
    "--bundle",
    "--format=esm",
    "--platform=browser",
    `--outfile=${join(fixtureRoot, "bundle.js")}`,
  ], { encoding: "utf8" });
  if (bundle.status !== 0) throw new Error(bundle.stderr || "Transform trust-ingress browser bundle failed.");
  ...
  if (address === null || typeof address === "string") throw new Error("Transform trust-ingress server has no TCP address.");
  ...
  const timed = new Promise((_, reject) => {
    timeoutId = setTimeout(() => reject(new Error(`Transform trust-ingress browser acceptance timed out.\n${stderr}`)), 30_000);
  });
  const exited = new Promise((_, reject) => chrome.once("close", (code) => {
    reject(new Error(`Chrome exited ${code} before reporting Transform trust-ingress acceptance.\n${stderr}`));
  }));
  const result = await Promise.race([browserResult, timed, exited]);
  if (result.status !== "pass") throw new Error(result.detail || "Transform trust-ingress browser acceptance failed.");
  process.stdout.write(`Transform trust-ingress browser acceptance passed (${result.detail}).\n`);
} finally {
