...
const source = 'const source = Hson.canonical`<user <age "37">>`;\n';
...
export const cases: Readonly<Record<string, string>> = {
  ...
  document: imports + 'const source = Hson.canonical`<button count="bad"/>`;\n' + map + 'map.schema.use(DocumentSchema);',
  documentMutated: imports + 'const source = Hson.canonical`<button count=1 disabled/>`;\n' + map + 'map.at([]).attrs.set("count", 2); map.schema.use(DocumentSchema);',
  documentReverted: imports + 'const source = Hson.canonical`<button count=1 disabled/>`;\n' + map + 'map.at([]).attrs.set("count", 2); map.at([]).attrs.set("count", 1); map.schema.use(DocumentSchema);',
  documentSequence: imports + 'const source = Hson.canonical`<a/> <c/>`;\n' + map + 'map.schema.use(DocumentSequenceSchema);',
  text: imports + 'const source = Hson.canonical`"text"`;\n' + map + 'map.schema.use(TextSchema);',
  scalar: imports + 'const source = Hson.canonical`"text"`;\n' + map + 'map.schema.use(StringSchema);',
  inline: imports.replace('import { Hson, hson } from "hson-live";', 'import { Hson } from "hson-live/hson";') + 'const map = hsonLiveMap.fromHson(Hson.canonical`<user <age "37">>`);\n' + use,
  ...
  ordered: imports + 'const source = Hson.canonical`<\'2\' "b" \'1\' "a">`;\n' + map + 'map.schema.use(OrderedSchema);',
  ...
  mixed: imports + source + map + use + 'try { OtherSchema.certify(source); } catch {}',
  repeated: imports + 'function make() {\n' + source + map + use + '} make(); make();',
  equal: imports + source + 'const equal = Hson.canonical`<user <age "37">>`;\n' + map + 'const b = hsonLiveMap.fromHson(equal);\n' + use + 'b.schema.use(OtherSchema);',
