...
import {
  hsonMirror,
} from "../src/api/mirror/mirror.facade.ts";
import {
...
export const HSON_LIVE_TEST_METADATA = Object.freeze({
  id: "mirror.document-attrs",
  title: "Document Mirror attributes",
  category: "Mirror",
  runtime: "node-synthetic-dom",
  tags: Object.freeze(["document", "binding", "attributes"]),
});

const testEvents = create_test_event_emitter("mirror.document-attrs");
let checks = 0;
...
check("projection failure is isolated from the committed map mutation", () => {
  ...
  assert.equal(binding.failure?.code, DOCUMENT_MIRROR_UPDATE_FAILED_ERROR_CODE);
...
check("a previously mounted node losing its DOM mapping fails closed", () => {
  ...
  assert.equal(commit.changed, true);
  assert.equal(binding.status, "failed");
  assert.equal(binding.failure?.code, "DOCUMENT_MIRROR_DOM_MAPPING_MISMATCH");
  binding.dispose();
});
...
check("projected path and persisted-QUID divergence fail closed", () => {
  ...
  assert.equal(quidBinding.failure?.code, DOCUMENT_MIRROR_QUID_MISMATCH_ERROR_CODE);
  quidBinding.dispose();
  ...
  assert.equal(pathBinding.failure?.code, DOCUMENT_MIRROR_TARGET_MISSING_ERROR_CODE);
  pathBinding.dispose();
});
...
check("cardinality and disposal preserve authority boundaries", () => {
  ...
  assert.throws(
    ...
    (cause) => cause instanceof DocumentMirrorError
      && cause.code === DOCUMENT_MIRROR_ALREADY_BOUND_ERROR_CODE,
  );
  binding.dispose();
  binding.dispose();
  ...
  assert.throws(
    () => binding.diagnostics(),
    (cause) => cause instanceof DocumentMirrorError
      && cause.code === DOCUMENT_MIRROR_DISPOSED_ERROR_CODE,
  );
