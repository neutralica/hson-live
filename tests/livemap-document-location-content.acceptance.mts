...
const fragment = (source: string): FragmentLiveMap => { const map = hson.liveMap.fromHson(source); if (map.mode !== "fragment") throw new Error("Expected fragment map"); return map; };
const emptyFragment = (): FragmentLiveMap => { const map = hson.liveMap.fromNode({ $_tag: "_hson_root", $_content: [] }); if (map.mode !== "fragment") throw new Error("Expected fragment map"); return map; };
const ordinary = (source: string): HsonNode => element(source).element.node();
...
check("fragment-root insert owns top-level content", () => { const map = fragment(`<a/> <c/>`); map.at([]).insert(1, ordinary(`<b/>`)); assert.deepEqual([0, 1, 2].map((i) => tag(map.at([i]).snap())), ["a", "b", "c"]); });
check("first empty-fragment text insert reuses canonical materialization", () => { const map = emptyFragment(); const equivalent = emptyFragment(); const receiver = emptyFragment(); const commit = map.at([]).insert(0, "first"); assert.equal(commit.ops[0]?.op, "insert-content"); assert.deepEqual(commit, equivalent.document.content.insert(target(), 0, "first")); receiver.replay(commit); assert.deepEqual(map.root(), equivalent.root()); assert.deepEqual(receiver.root(), map.root()); assert.equal(map.at([0]).snap(), "first"); });
...
check("fragment move owns top-level content", () => { const map = fragment(`<a/> <b/> <c/>`); map.at([]).move(2, 0); assert.deepEqual([0, 1, 2].map((i) => tag(map.at([i]).snap())), ["c", "a", "b"]); });
...
check("insert commit exactly equals document API", () => { const left = element(`<main <a/>/>`); const right = element(`<main <a/>/>`); const value = ordinary(`<b/>`); assert.deepEqual(left.at([]).insert(1, value), right.document.content.insert(target(0), 1, value)); assert.deepEqual(left.root(), right.root()); });
check("move commit exactly equals document API", () => { const left = element(`<main <a/> <b/>/>`); const right = element(`<main <a/> <b/>/>`); assert.deepEqual(left.at([]).move(0, 1), right.document.content.move(target(0), 0, 1)); assert.deepEqual(left.root(), right.root()); });
