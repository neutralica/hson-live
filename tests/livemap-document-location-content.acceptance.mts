...
check("Reflection consumes location content commits", () => { const map = reflectedElement(`<main <a/>/>`); const binding = _reflect_document_for_runtime_test(_create_livetree_runtime_test_handle(), map); map.at([]).insert(1, ordinary(`<b/>`)); assert.equal(raw_node(binding.tree.node, [0, 0, 1]).$_tag, "b"); binding.dispose(); });
