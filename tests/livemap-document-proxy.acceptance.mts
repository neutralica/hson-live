...
check("schema evidence does not change proxy locations or interning", () => {
  ...
  const schema = hson.liveMap.schema.define((s) => s.tag(s.tag(s.string)));
...
check("document $_ exposes the established location capability surface", () => {
  ...
  assert.deepEqual(Reflect.ownKeys(location).sort(), ["at", "attrs", "delete", "flags", "id", "insert", "move", "path", "replace", "rev", "snap", "watch"]);
