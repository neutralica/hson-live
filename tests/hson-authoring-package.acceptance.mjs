...
const sources = {
  ...
  document: 'import { Hson } from "hson-live/hson"; const empty=Hson.document.fromHson(""); const text=Hson.document.fromHson(String.fromCharCode(34,34)); const value=Hson.document`<main @000000001/><aside/>`; export const parity={empty,distinct:empty!==text,root:Hson.document.toNode(value).$_tag,items:Hson.document.toNode(value).$_content.length,roundTrip:Hson.document.fromHson(value)===value,primitive:typeof value==="string"};',
  action: 'import { Hson, hson } from "hson-live"; const clients=new Set(),servers=new Set(); const client={send(v){for(const f of servers)f(v)},close(){},onMessage(f){clients.add(f);return()=>clients.delete(f)},onClose(){return()=>{}}}; const server={send(v){for(const f of clients)f(v)},close(){},onMessage(f){servers.add(f);return()=>servers.delete(f)},onClose(){return()=>{}}}; let executions=0; const locus=hson.locus.create({state:{},actions:{echo(_c,p){executions++;return p}}}); locus.connect(server); const echo=hson.echo.create({socket:client}); echo.connect(); await echo.session.create(); const map=hson.liveMap.fromHson(Hson.canonical`<payload <\'10\' -0 \'2\' <__proto__ true> tail <b 2 a 1>>>`); const value=map.at(["payload"]).data(); const outcome=await echo.action("echo",value); let reservedRejected=false; try{echo.action("echo",{_hson_root:true})}catch{reservedRejected=true} const entries=Hson.data.entries(value); export const parity={ack:outcome.type==="ack",equal:outcome.type==="ack"&&outcome.result===value,order:entries.map(([name])=>name),negativeZero:Object.is(Hson.data.materialize(entries[0][1]),-0),safeProto:Object.hasOwn(Hson.data.materialize(entries[1][1]),"__proto__"),reservedRejected,executions}; echo.dispose(); locus.dispose();',
