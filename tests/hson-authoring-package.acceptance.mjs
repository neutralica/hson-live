...
const sources = {
  ...
  document: 'import { HsonDocument } from "hson-live/hson"; const empty=HsonDocument.fromHson(""); const text=HsonDocument.fromHson("\\\"\\\""); const value=HsonDocument.fromHson("<main @000000001/><aside/>"); export const parity={empty:empty.toHson(),distinct:!empty.equals(text),root:value.toNode().$_tag,items:value.toNode().$_content.length,roundTrip:HsonDocument.fromHson(value.toHson()).equals(value),frozen:Object.isFrozen(value)};',
...
check("browser-targeted authoring executes exact HsonDocument semantics", () => assert.deepEqual(documentExecution.parity, { empty: "", distinct: true, root: "_hson_root", items: 2, roundTrip: true, frozen: true }));
