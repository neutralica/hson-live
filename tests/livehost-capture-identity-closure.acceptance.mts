...
} from "../src/api/livehost/livehost.graph-content-codec.ts";
import {
  decode_livehost_document_snapshot,
  encode_livehost_document_snapshot,
} from "../src/api/livehost/livehost.document-snapshot.ts";
import {
  capture_livehost_bootstrap,
  install_livehost_bootstrap,
} from "../src/api/livehost/livehost.bootstrap.ts";
import {
...
check("graph-content codec preserves detached QUID metadata", () => {
  ...
  const decoded = decode_livehost_graph_content(encode_livehost_graph_content(node));
...
check("graph-content admission creates fresh map-local lookup", () => {
  const decoded = decode_livehost_graph_content(
    encode_livehost_graph_content(projected_element(`<i @${Q1}/>`)),
  );
...
check("graph-content bytes never mint absent QUIDs", () => {
  const decoded = decode_livehost_graph_content(encode_livehost_graph_content(projected_element(`<i/>`)));
  assert.equal(JSON.stringify(decoded).includes("quid"), false);
});

check("view-state LiveHost snapshots retain exact metadata", () => {
  const source = element(`<main @${Q1}/>`);
...
check("HSON LiveHost snapshots retain exact metadata", () => {
  const source = element(`<main @${Q1}/>`);
  const snapshot = encode_livehost_document_snapshot(
...
check("LiveHost bootstrap installs metadata into a new mirror epoch", () => {
  const source = element(`<main @${Q1}/>`);
  const host = hson.liveHost.create({ map: source, logicalMapId: "unit7-bootstrap" });
  const bootstrap = capture_livehost_bootstrap(host, "unit7:bootstrap", "/unit7");
...
check("bootstrap logical identity is not same-epoch node provenance", () => {
  const source = element(`<main @${Q1}/>`);
  const capability = source.capture({ identity: "same-epoch" });
  const host = hson.liveHost.create({ map: source, logicalMapId: "unit7-bootstrap-proof" });
  const installed = install_livehost_bootstrap(capture_livehost_bootstrap(host, "unit7:proof", "/unit7-proof"));
...
process.stdout.write(`# ${checks} LiveHost and Reflection capture-identity closure checks passed\n`);
emit_hson_live_test_completion("livehost.capture-identity-closure", checks, checks, 0);
