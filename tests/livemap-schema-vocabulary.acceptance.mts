...
check("structured constrain preserves and does not transform base evidence", () => { const State = hson.liveMap.schema.define((s) => s.exact({ count: s.number }).constrain("positive count", (value) => value.count > 0)); const map = hson.liveMap.fromJson({ count: 2 }).schema.use(State); assert.deepEqual(map.snap(), { count: 2 }); });
...
check("constrain then optional or nullable bypasses the predicate for absence and null", () => { let calls = 0; const State = hson.liveMap.schema.define((s) => s.exact({ optional: s.number.constrain((value) => { calls += 1; return value > 0; }).optional, nullable: s.number.constrain((value) => { calls += 1; return value > 0; }).nullable })); assert.equal(State.validateRoot({ nullable: null }).ok, true); assert.equal(calls, 0); });
check("optional then constrain preserves the established required-present result", () => { const State = hson.liveMap.schema.define((s) => s.exact({ value: s.number.optional.constrain((value) => value > 0) })); assert.equal(State.validateRoot({}).issues[0]?.code, "MISSING_REQUIRED"); assert.equal(State.validateRoot({ value: 1 }).ok, true); });
