...
check("exact schemas see no unknown identity key", () => { const schema = hson.liveMap.schema.define((s) => s.exact({ x: s.exact({ value: s.number }) })); const a = map({ x: { value: 1 } }).withSchema(schema); acquire_projected_identity(a, ["x"]); assert.deepEqual(a.snap(), { x: { value: 1 } }); });
check("readonly schemas still observe the same projected value", () => { const schema = hson.liveMap.schema.define((s) => s.exact({ x: s.exact({ value: s.number.readonly }) })); const a = map({ x: { value: 1 } }).withSchema(schema); const before = a.snap(); acquire_projected_identity(a, ["x"]); assert.deepEqual(a.snap(), before); assert.equal(a.schema.get(), schema); });
