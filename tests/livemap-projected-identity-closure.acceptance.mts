...
check("projected feeds do not publish hidden metadata-only changes", () => { const a = map({ x: {} }); let seen = 0; a.feed(["x"], () => seen += 1); acquire_projected_identity(a, ["x"]); assert.equal(seen, 0); });
...
check("schemas still observe the same projected value", () => { const schema = hson.liveMap.schema.define((s) => s.object.exact({ x: s.object.exact({ value: s.number }) })); const a = map({ x: { value: 1 } }).schema.use(schema); const before = a.snap(); acquire_projected_identity(a, ["x"]); assert.deepEqual(a.snap(), before); assert.equal(a.schema.get(), schema); });
