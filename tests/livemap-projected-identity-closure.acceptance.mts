...
check("colliding replay rejects atomically", () => { const b = map({ x: {}, y: {} }); set_livemap_projected_quid_candidate_source_for_tests(b, () => Q1); acquire_projected_identity(b, ["x"]); const before = b.root(); assert.throws(() => b.replay({ changed: true, prevRev: 1, rev: 2, ops: [{ domain: "graph", op: "ensure-quid", target: { kind: "path", path: ["y"], projected: true }, quid: Q1 }] } as never)); assert.equal(canonical_hson_graph_equal(before, b.root()), true); });
check("canonical commit observers see identity registration", () => { const a = map({}); let seen = 0; a.commits.observe((e) => { if (e.kind === "commit" && e.commit.ops[0] && "domain" in e.commit.ops[0]) seen += 1; }); acquire_projected_identity(a, []); assert.equal(seen, 1); });
check("data feeds do not publish hidden metadata-only changes", () => { const a = map({ x: {} }); let seen = 0; a.feed(["x"], () => seen += 1); acquire_projected_identity(a, ["x"]); assert.equal(seen, 0); });
check("stores do not publish hidden metadata-only changes", () => { const a = map({ x: {} }); const store = make_livemap_store_api(a); let seen = 0; store.subscribe(() => seen += 1); acquire_projected_identity(a, ["x"]); assert.equal(seen, 0); });
check("links remain projected-value channels and do not claim exact identity", () => { const a = map({ x: {} }); const b = map({ x: {} }); const stop = link_livemap(a, b, { path: ["x"] }); acquire_projected_identity(a, ["x"]); assert.equal(quidAt(b, ["x"]), undefined); stop(); });
check("exact schemas see no unknown identity key", () => { const schema = hson.liveMap.schema.define((s) => s.object.exact({ x: s.object.exact({ value: s.number }) })); const a = map({ x: { value: 1 } }).schema.use(schema); acquire_projected_identity(a, ["x"]); assert.deepEqual(a.snap(), { x: { value: 1 } }); });
check("schemas still observe the same data value", () => { const schema = hson.liveMap.schema.define((s) => s.object.exact({ x: s.object.exact({ value: s.number }) })); const a = map({ x: { value: 1 } }).schema.use(schema); const before = a.snap(); acquire_projected_identity(a, ["x"]); assert.deepEqual(a.snap(), before); assert.equal(a.schema.get(), schema); });
check("map.at remains location-bound after retained value moves", () => { const a = map({ x: {}, y: { next: true } }); const location = a.at(["x"]); const identity = acquire_projected_identity(a, ["x"]); a.at([]).object.renameKey("x", "z"); assert.equal(location.snap(), undefined); assert.deepEqual(identity.path(), ["z"]); });
...
check("QUID-free large data maps retain zero claims", () => { const rows = Array.from({ length: 2000 }, (_, id) => ({ id, nested: {} })); const a = map({ rows }); a.snap(); a.at(["rows"]).array.move(0, 1999); assert.equal(collect_hson_node_quid_claims(a.root()).length, 0); });
check("one internal acquisition adds only one sparse claim", () => { const rows = Array.from({ length: 1000 }, (_, id) => ({ id })); const a = map({ rows }); acquire_projected_identity(a, ["rows", 500]); assert.equal(collect_hson_node_quid_claims(a.root()).length, 1); });
check("identity-specific reconciliation visits sparse entries", () => { const a = map({ rows: [{}, {}, {}] }); acquire_projected_identity(a, ["rows", 1]); const before = livemap_projected_identity_accounting(); a.at(["rows"]).array.move(1, 2); const after = livemap_projected_identity_accounting(); assert.equal(after.overlayEntriesVisited - before.overlayEntriesVisited, 1); });
check("ordinary data mutations never allocate identity", () => { const a = map({ x: {}, rows: [{}] }); set_livemap_projected_quid_candidate_source_for_tests(a, () => { throw new Error("minted"); }); a.set(["x"], { value: 1 }); a.at(["rows"]).array.push({}); assert.equal(collect_hson_node_quid_claims(a.root()).length, 0); });
...
check("Locus Hson recovery preserves projected container metadata as fresh local identity", () => {
  ...
  if (b.mode === "element" || b.mode === "fragment") throw new Error("expected data snapshot");
  ...
  assert.equal(acquire_projected_identity(b, ["x"]).active, true);
  assert.equal(acquire_projected_identity(b, ["y"]).active, true);
  plan.dispose();
});
