...
check("disposal does not remove the shared overlay claim", () => { const owner = map({}); const h = acquire_projected_identity(owner, []); h.dispose(); const rev = owner.rev; const replacement = acquire_projected_identity(owner, []); assert.equal(replacement.active, true); assert.equal(owner.rev, rev); });
