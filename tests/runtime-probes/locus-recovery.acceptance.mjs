...
function expect_recovery_error(fn, code) {
  try {
    fn();
    assert.fail(`Expected Locus recovery error ${code}.`);
  } catch (cause) {
    assert.ok(cause instanceof LocusRecoveryError);
...
function recovery_host(options) {
  ...
  const stream = make_locus_canonical_stream(map, {
    ...(options.history === undefined ? {} : { history: options.history }),
  });
  const recovery = make_locus_recovery_planner(map, stream, options.recovery);
