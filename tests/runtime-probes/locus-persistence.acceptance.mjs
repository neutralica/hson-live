...
await check("initial checkpoint failure rolls back exclusive management", async () => {
  ...
  await assert.rejects(
    create_persistent_locus({ map, persistence: adapter }),
    (cause) => cause instanceof LocusPersistenceError
      && cause.code === "LOCUS_PERSISTENCE_INITIAL_CHECKPOINT_FAILED",
  );
...
await check("append failure is inert and a later queued mutation succeeds", async () => {
  ...
  await assert.rejects(
    ...
    (cause) => cause instanceof LocusPersistenceError && cause.code === "LOCUS_PERSISTENCE_APPEND_FAILED",
  );
  assert.equal(map.rev, 0);
  assert.equal(host.stream.headRev, 0);
...
await check("post-append realization failure terminally fences memory and reload realizes durable lineage", async () => {
  const adapter = new MemoryPersistenceAdapter();
  const map = element();
  const host = await create_persistent_locus_internal({
    map,
    persistence: adapter,
...
await check("checkpoint failure preserves the prior durable chain and host health", async () => {
  ...
  await assert.rejects(host.checkpoint(), (cause) => cause instanceof LocusPersistenceError
    && cause.code === "LOCUS_PERSISTENCE_CHECKPOINT_FAILED");
...
await check("exclusive actions persist through the gate and projected persistence is deferred", async () => {
  ...
  await assert.rejects(
    create_persistent_locus({ map: targetMap, persistence: projectedAdapter }),
    (cause) => cause instanceof LocusPersistenceError
      && cause.code === "LOCUS_PERSISTENCE_MAP_KIND_UNSUPPORTED",
  );
  source.set(["value"], 1);
...
await check("ordinary constructor and data maps cannot opt into persistence", async () => {
  ...
  await assert.rejects(
    create_persistent_locus({ map: hson.liveMap.fromJson({ value: 0 }), persistence: adapter }),
    (cause) => cause instanceof LocusPersistenceError
      && cause.code === "LOCUS_PERSISTENCE_MAP_KIND_UNSUPPORTED",
  );
});
...
await check("corrupt persisted envelopes and tails reject without partial registration", async () => {
  ...
  for (const corrupt of corruptions) {
    ...
    assert.equal(result.error.code, "LOCUS_PERSISTED_STATE_INVALID");
