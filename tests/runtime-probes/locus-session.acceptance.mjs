...
export const HSON_LIVE_TEST_METADATA = Object.freeze({
  ...
  tags: Object.freeze(["session", "recovery", "websocket"]),
});

const testEvents = create_test_event_emitter("locus.session");
let checks = 0;
...
await check("replacement during recovery prevents the old connection from reaching caught-up", async () => {
  ...
  await assert.rejects(interrupted, (error) => error.code === "LOCUS_SESSION_ATTACHMENT_FENCED");
  await attached;
...
await check("graceful goodbye revokes credentials but leaves map state available", async () => {
  ...
  await assert.rejects(first.client.session.goodbye(), (error) => error.code === "LOCUS_SESSION_ALREADY_GONE");
  const revoked = connect_client(host, { session: { credential } });
  await assert.rejects(revoked.client.session.reattach(), (error) => error.code === "LOCUS_SESSION_CREDENTIAL_REVOKED");
...
await check("missing malformed and unknown credentials are classified", async () => {
  ...
  for (const [credential, code] of [
    [undefined, "LOCUS_SESSION_CREDENTIAL_MISSING"],
    ["short", "LOCUS_SESSION_CREDENTIAL_MALFORMED"],
    ["unknown-credential-value", "LOCUS_SESSION_CREDENTIAL_UNKNOWN"],
  ]) {
  ...
  assert.equal(counts.LOCUS_SESSION_CREDENTIAL_MISSING, 1);
  assert.equal(counts.LOCUS_SESSION_CREDENTIAL_MALFORMED, 1);
  assert.equal(counts.LOCUS_SESSION_CREDENTIAL_UNKNOWN, 1);
});
