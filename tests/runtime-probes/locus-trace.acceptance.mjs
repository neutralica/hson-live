...
import { create_live_trace_context } from "../../src/api/locus/locus.trace.ts";
...
export const HSON_LIVE_TEST_METADATA = Object.freeze({
  ...
  category: "Locus",
  runtime: "node",
  tags: Object.freeze(["trace", "actions", "redaction"]),
});

const testEvents = create_test_event_emitter("locus.trace");
let checks = 0;
...
await check("publication failure retains action causation and emits one aggregate failure", async () => {
  ...
  assert.equal(publications[0].details.errorCode, "LOCUS_COMMIT_PUBLICATION_FAILED");
...
await check("schema rejection traces the rejecting stage without reaching handler or mutation", async () => {
  ...
  assert.equal(result.error.code, "LOCUS_SCHEMA_INVALID_PAYLOAD");
  ...
  assert.equal(rejection?.details?.errorCode, "LOCUS_SCHEMA_INVALID_PAYLOAD");
...
await check("console sink is explicit, compact, correlated, and redacts unsafe text", () => {
  ...
  sink.emit({
    ...
    timestamp: 10,
    durationMs: 2,
    details: { action: "update", errorCode: "LOCUS_SCHEMA_INVALID_PAYLOAD", payload: "console-secret" },
  });
  assert.equal(lines.length, 1);
  assert.match(lines[0], /^\[trace console-trace #07\] locus payload\.validation failure 2\.0ms/);
  assert.match(lines[0], /action=update/);
  assert.match(lines[0], /errorCode=LOCUS_SCHEMA_INVALID_PAYLOAD/);
...
await check("host trace identity is distinct per processing attempt despite retries and reused client attempt IDs", async () => {
  ...
  assert.ok(traceIds.every((traceId) => traceId.startsWith("locus-trace-")));
  const serialized = JSON.stringify(events);
