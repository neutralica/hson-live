    ...
    actions: {
      echo: {
        payload(value) {
          ...
          if (!(value instanceof HsonData)) return { ok: false, issues: ["not exact data"] };
          schemaValue = value;
  ...
  authorizeAction(context) {
    ...
    if (context.action === "echo" && context.payload?.kind === "object") {
      ...
      if (names?.includes("__proto__")) {
        const ordinary = context.payload.materialize() as Record<string, unknown>;
...
const map = hson.liveMap.fromHson(Hson`<payload <'10' -0 '2' <nested <__proto__ <constructor 1 prototype 2>>> __proto__ <polluted true> tail [0,-0]>>`);
if (map.mode === "document") throw new Error("Expected data LiveMap.");
const source = map.at(["payload"]).data();
assert.ok(source instanceof HsonData);
...
assert.equal(source.equals(schemaValue as HsonData), true);
assert.equal(schemaValue, authorizationValue);
assert.equal(authorizationValue, handlerValue);
assert.equal(source.equals(first.result), true);
assert.deepEqual(source.entries()?.map(([name]) => name), ["10", "2", "__proto__", "tail"]);
assert.equal(Object.is(source.entries()?.[0]?.[1].scalar(), -0), true);
assert.equal(Object.hasOwn(source.entries()?.[1]?.[1].entries()?.[0]?.[1].materialize() as object, "__proto__"), true);
assert.equal(source.entries()?.[2]?.[0], "__proto__");
...
assert.equal(cached.result.equals(first.result), true);
...
assert.equal(status.outcome.result.equals(first.result), true);

const mutable = { nested: { value: 1 } };
const snapshotted = echo.action("echo", mutable);
mutable.nested.value = 9;
assert.equal((snapshotted.request.payload as HsonData).entries()?.[0]?.[1].entries()?.[0]?.[1].scalar(), 1);
const snapshottedResult = await snapshotted;
assert.equal(snapshottedResult.type === "ack" && snapshottedResult.result?.entries()?.[0]?.[1].entries()?.[0]?.[1].scalar(), 1);

const orderConflict = await echo.retryAction({
  requestId: call.request.requestId,
  name: "echo",
  payload: HsonData.fromHson(Hson`<'2' <nested <__proto__ <constructor 1 prototype 2>>> '10' -0 __proto__ <polluted true> tail [0,-0]>`),
});
...
assert.equal(observableResults[1].type === "ack" && observableResults[1].result?.kind, "null");
assert.equal(Object.is(observableResults[2].type === "ack" && observableResults[2].result?.scalar(), 0), true);
assert.equal(Object.is(observableResults[3].type === "ack" && observableResults[3].result?.scalar(), -0), true);
assert.equal(observableResults[4].type === "ack" && observableResults[4].result?.kind, "object");
assert.deepEqual(observableResults[4].type === "ack" && observableResults[4].result?.entries(), []);
assert.equal(observableResults[5].type === "ack" && observableResults[5].result?.kind, "array");
assert.deepEqual(observableResults[5].type === "ack" && observableResults[5].result?.items(), []);
...
if (transformedExact.type === "ack") {
  assert.deepEqual(transformedExact.result?.entries()?.map(([name]) => name), ["z", "y"]);
  assert.equal(Object.is(transformedExact.result?.entries()?.[0]?.[1].scalar(), -0), true);
}
