...
await check("public endpoint-only Echo uses explicit session lifecycle without replica state", async () => {
  ...
  const locus = create_locus({
    map: hsonLiveMap.fromLibraries({ state: { data: { value: 0 }, schema: Hson.schema`<type "data" content <value "number">>` } }),
    exposure: [{ library: "state", exposure: "server-private" }],
    ...
    actions: {
      async increment(context, payload) {
        ...
        const state = context.map.lib("state");
        const value = state.snap(["value"]);
        if (typeof value !== "number") throw new Error("Expected numeric authority state.");
        await context.mutate((draft) => draft.lib("state").at(["value"]).set(value + by));
      },
    },
  });
  ...
  const state = locus.map.lib("state");
