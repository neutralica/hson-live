...

{
  ...
  await assert.rejects(
    continue_hosted_document({ echo, root: fixture.root as unknown as Element } as never),
    /requires a projected authority snapshot/i,
  );
  assert.equal(get_node_for_el(fixture.root as unknown as Element), undefined);
  const continuation = await continue_legacy_hosted_document({ echo, root: fixture.root as unknown as Element });
...

{
  ...
  const fixture = mainFixture(quid);
  await assert.rejects(
    continue_legacy_hosted_document({ echo, root: fixture.root as unknown as Element }),
    (cause) => cause instanceof DocumentContinuationError && cause.phase === "recover" && cause.cause !== undefined,
  );
  assert.equal(get_node_for_el(fixture.root as unknown as Element), undefined);
  echo.connect();
  await echo.session.create();
  const retry = await continue_legacy_hosted_document({ echo, root: fixture.root as unknown as Element });
