...
export const HSON_LIVE_TEST_METADATA = Object.freeze({
  id: "locus.authority",
  title: "Locus authority",
  category: "Locus",
  runtime: "node",
  tags: Object.freeze(["authority", "lifecycle", "commit"]),
});

const testEvents = create_test_event_emitter("locus.authority");
let checks = 0;
...
await check("gate rejection is inert and releases the queue for the next request", async () => {
  ...
  await assert.rejects(rejected, (cause) => cause instanceof LocusAuthorityError && cause.code === "LOCUS_AUTHORITY_GATE_REJECTED");
  assert.equal(map.rev, 0);
  assert.equal(host.stream.headRev, 0);
  await tick();
  gates.calls[1].resolve();
...
await check("exclusive actions track awaited and unawaited queued mutations", async () => {
  ...
  const host = create_locus_internal({
    map,
        actions: {
      set: (ctx, payload) => { void ctx.mutate((draft) => draft.set(["value"], Hson.data.materialize(payload).value)); },
      twice: async (ctx) => {
...
await check("managed link targets enqueue without blocking source acceptance", async () => {
  ...
  const gates = deferred_gates();
  const host = create_locus_internal({ map: target }, { authorityGate: gates.gate });
  source.set(["value"], 4);
  await tick();
...
await check("same-host and cross-host managed links queue separate commits without deadlock", async () => {
  ...
  const selfHost = hson.locus.create({ map: selfMap });
  ...
  const leftHost = hson.locus.create({ map: left });
  const rightHost = hson.locus.create({ map: right });
...
await check("authority traces expose lifecycle metadata without mutation content", async () => {
  ...
  const host = hson.locus.create({ map, trace });
...
await check("post-gate acceptance invariant failure is terminal", async () => {
  ...
  await assert.rejects(
    ...
    (cause) => cause instanceof LocusAuthorityError && cause.code === "LOCUS_AUTHORITY_TERMINAL",
  );
  await assert.rejects(
  ...
  assert.throws(
    ...
    (cause) => cause?.code === "LOCUS_RECOVERY_DISPOSED",
  );
  host.dispose();
});
...
await check("accepted history-ingestion failure is classified and fences later authority", async () => {
  ...
  await assert.rejects(
    host.mutate((draft) => draft.set(["value"], 1)),
    (cause) => cause instanceof LocusAuthorityError
      && cause.code === "LOCUS_AUTHORITY_ACCEPTED_INGESTION_FAILED",
  );
  assert.equal(map.rev, 1);
  assert.equal(host.stream.headRev, 0);
...
await check("host destruction rejects queued work and releases management after active gate settles", async () => {
  ...
  await tick();
  host.dispose();
  await assert.rejects(queued, (cause) => cause instanceof LocusAuthorityError && cause.code === "LOCUS_AUTHORITY_CLOSED");
...
await check("single-host ownership conflicts and release are controlled", async () => {
  const map = hson.liveMap.fromJson({ value: 0 });
  const first = hson.locus.create({ map });
  assert.throws(
    () => hson.locus.create({ map }),
    (cause) => cause instanceof LocusAuthorityError && cause.code === "LOCUS_AUTHORITY_ALREADY_MANAGED",
  );
  first.dispose();
  await tick();
  const replacement = hson.locus.create({ map });
