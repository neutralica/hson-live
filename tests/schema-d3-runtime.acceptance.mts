...
await check('data string Schema is not silently used to reinterpret a document', async () => { assert.equal(evidence('scalar')[0]!.rootMode,'fragment'); assert.equal((await association('scalar')).result.result?.status,'INVALID'); });
