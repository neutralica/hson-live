...
check("projected object cannot use element-only capability", () => assert.throws(() => root.HSON.validate(root.hson.liveMap.schema.define(s => s.button()), root.HSON`<age 37>`), map.LiveMapSchemaError));
...
check("combined tuple capability validates projected array", () => { const schema = root.hson.liveMap.schema.define(s => s.tuple(s.string, s.string)); const canonical = root.HSON`["a", "b"]`; assert.equal(root.HSON.validate(schema, canonical), canonical); });
