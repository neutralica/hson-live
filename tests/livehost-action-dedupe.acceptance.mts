...
function fixture(options = {}) {
  ...
  const host = hson.liveHost.create({
    ...
    actions: {
      ...
      echo(_ctx, value) { executions += 1; return value; },
      fail() { executions += 1; throw new Error("expected failure"); },
      mutateFail(ctx, value) { executions += 1; ctx.map.set(["value"], value); throw new Error("after mutation"); },
      noop() { executions += 1; return { ok: true }; },
      invalidResult() { executions += 1; return new Date(); },
      async independent(ctx, value) { executions += 1; entered.resolve(); await gate.promise; ctx.map.set(["value"], value); return value; },
      reentrant(_ctx, value) { executions += 1; reentrant?.(); return value; },
    },
  });
  return { host, gate, entered, completed, executions: () => executions, setReentrant(fn) { reentrant = fn; } };
}

await check("new request installs pending and returns request ID plus completion revision", async () => {
  const f = fixture();
  ...
  assert.equal((await client.action_status(call.request.requestId)).state, "pending");
  f.gate.resolve();
...
await check("request fingerprint conflict never starts a second execution", async () => {
  const f = fixture();
  const { client } = connect(f.host, "conflict-client");
  const original = client.action("gated", 5);
  await f.entered.promise;
  const conflict = await client.retry_action({ requestId: original.request.requestId, name: "gated", payload: 6 });
  assert.equal(conflict.type, "error");
  assert.equal(conflict.error.code, "LIVEHOST_ACTION_REQUEST_ID_CONFLICT");
  assert.equal(conflict.delivery, "rejected");
  assert.equal(f.executions(), 1);
  const nameConflict = await client.retry_action({ requestId: original.request.requestId, name: "echo", payload: 5 });
  assert.equal(nameConflict.error.code, "LIVEHOST_ACTION_REQUEST_ID_CONFLICT");
  assert.equal(f.executions(), 1);
  f.gate.resolve();
...
await check("overlapping independent request IDs execute without global serialization", async () => {
  const f = fixture();
...
await check("disconnect uncertainty resolves through status and cached retry in a new session", async () => {
  const f = fixture();
  ...
  assert.equal((await b.client.action_status(request.requestId)).state, "pending");
...
await check("failed action is terminal and cached", async () => {
  ...
  assert.equal(f.executions(), 1);
  assert.equal((await client.action_status(first.request.requestId)).state, "failed");
});

await check("mutation then failure preserves commit and completion revision without rerun", async () => {
  const f = fixture();
...
await check("no-op success retains the current authoritative head", async () => {
  const f = fixture();
  ...
  assert.equal((await client.retry_action(first.request)).completionRev, before);
  assert.equal(f.executions(), 1);
});

await check("independent IDs and browser clients execute independently", async () => {
  const f = fixture();
  const a = connect(f.host, "client-a", { actionId: () => "shared-request-id" });
  const b = connect(f.host, "client-b", { actionId: () => "shared-request-id" });
...
await check("same identity is isolated by map incarnation", async () => {
  ...
  const clientOne = connect(one.host, "same-browser", { actionId: () => "same-request" }).client;
  const clientTwo = connect(two.host, "same-browser", { actionId: () => "same-request" }).client;
...
await check("canonical fingerprint ignores object insertion order", async () => {
  const f = fixture();
  const { client } = connect(f.host, "canonical-client");
  const first = client.action("echo", { a: 1, nested: { x: 2, y: 3 } });
  await first;
  const cached = await client.retry_action({ requestId: first.request.requestId, name: "echo", payload: { nested: { y: 3, x: 2 }, a: 1 } });
  assert.equal(cached.delivery, "cached");
  assert.equal(f.executions(), 1);
});
...
await check("outcome normalization failure is retained", async () => {
  ...
  assert.equal(outcome.error.code, "LIVEHOST_ACTION_OUTCOME_NORMALIZATION_FAILED");
  assert.equal((await client.retry_action(first.request)).delivery, "cached");
  assert.equal(f.executions(), 1);
  assert.equal(f.host.actionRequests.debug().outcomeNormalizationFailureCount, 1);
});

await check("malformed identity and unavailable or invalid actions are classified", async () => {
  const f = fixture();
  const raw = connect(f.host, "identity-client");
  raw.pair.client.send(JSON.stringify({ type: "action", id: "attempt-missing", clientId: "identity-client", name: "echo", payload: 1 }));
  assert.equal(JSON.parse(raw.pair.serverSent.at(-1)).error.code, "LIVEHOST_ACTION_REQUEST_ID_MISSING");
  raw.pair.client.send(JSON.stringify({ type: "action", id: "attempt-malformed", requestId: "", clientId: "identity-client", name: "echo", payload: 1 }));
  assert.equal(JSON.parse(raw.pair.serverSent.at(-1)).error.code, "LIVEHOST_ACTION_REQUEST_ID_MALFORMED");
  const unavailable = await raw.client.action("does-not-exist", 1);
  assert.equal(unavailable.error.code, "LIVEHOST_ACTION_UNAVAILABLE");

  const invalidHost = hson.liveHost.create({
    state: { value: 0 },
    actions: { echo(_ctx, value) { return value; } },
    schema: { actions: { echo: { payload: (value) => typeof value === "number" } } },
  });
  const invalidClient = connect(invalidHost, "invalid-client").client;
  const invalid = await invalidClient.action("echo", "not-a-number");
  assert.equal(invalid.error.code, "LIVEHOST_ACTION_INVALID");
  assert.equal(invalidHost.actionRequests.debug().executionsStarted, 0);
});

await check("count byte and time retention produce explicit expired or unknown results", async () => {
  const clock = fake_clock();
  const count = fixture({ actionDedupe: { maxTerminalRecords: 1, maxTerminalBytes: 1_000_000, terminalRetentionMs: 1_000, maxExpiredTombstones: 2, now: clock.now, schedule: clock.schedule } });
  const client = connect(count.host, "retention-client").client;
  const first = client.action("echo", "one"); await first;
  const second = client.action("echo", "two"); await second;
  assert.equal((await client.action_status(first.request.requestId)).state, "expired");
  assert.equal((await client.retry_action(first.request)).error.code, "LIVEHOST_ACTION_REQUEST_EXPIRED");
  clock.advance(1_001);
  assert.equal((await client.action_status(second.request.requestId)).state, "expired");

  const bytes = fixture({ actionDedupe: { maxTerminalRecords: 10, maxTerminalBytes: 1, terminalRetentionMs: 1_000, maxExpiredTombstones: 1, now: clock.now, schedule: clock.schedule } });
  const byteClient = connect(bytes.host, "byte-client").client;
  const byteRequest = byteClient.action("echo", "large-result"); await byteRequest;
  assert.equal((await byteClient.action_status(byteRequest.request.requestId)).state, "expired");

  const forgotten = fixture({ actionDedupe: { maxTerminalRecords: 0, maxTerminalBytes: 0, maxExpiredTombstones: 1, now: clock.now, schedule: clock.schedule } });
  const forgottenClient = connect(forgotten.host, "forgotten-client").client;
  const old = forgottenClient.action("echo", 1); await old;
  const newer = forgottenClient.action("echo", 2); await newer;
  assert.equal((await forgottenClient.action_status(old.request.requestId)).state, "unknown");
  const unknownRetry = await forgottenClient.retry_action(old.request);
  assert.equal(unknownRetry.error.code, "LIVEHOST_ACTION_REQUEST_UNKNOWN");
  assert.equal(forgotten.executions(), 2);
});

await check("terminal limits never evict a pending action", async () => {
  const clock = fake_clock();
  const f = fixture({ actionDedupe: { maxTerminalRecords: 0, maxTerminalBytes: 0, maxExpiredTombstones: 1, now: clock.now, schedule: clock.schedule } });
  const { client } = connect(f.host, "pending-retention");
  const pending = client.action("gated", 1);
  await f.entered.promise;
  await client.action("echo", 2);
  assert.equal((await client.action_status(pending.request.requestId)).state, "pending");
  assert.equal(f.host.actionRequests.debug().pendingRequestCount, 1);
  f.gate.resolve();
  await pending;
});

await check("reentrant identical request joins after pending installation", async () => {
  const f = fixture();
  const a = connect(f.host, "reentrant-client", { actionId: () => "reentrant-request" });
  const b = connect(f.host, "reentrant-client");
  const descriptor = { requestId: "reentrant-request", name: "reentrant", payload: 12 };
  let joined;
  f.setReentrant(() => { joined = b.client.retry_action(descriptor); });
...
await check("fenced transport cannot create a dedupe record", async () => {
  ...
  const second = connect(f.host, "fenced-client", { session: { credential } });
  await second.client.session.reattach();
  first.pair.client.send(JSON.stringify({ type: "action", id: "fenced-id", requestId: "fenced-id", clientId: "fenced-client", name: "echo", payload: 1 }));
  assert.equal(f.executions(), 0);
  assert.equal((await second.client.action_status("fenced-id")).state, "unknown");
});

await check("dedupe store disposal releases waiters and stays unavailable", async () => {
  const f = fixture();
  ...
  assert.equal(outcome.error.code, "LIVEHOST_ACTION_DEDUPE_STORE_UNAVAILABLE");
  const retry = await client.retry_action(pending.request);
  assert.equal(retry.error.code, "LIVEHOST_ACTION_DEDUPE_STORE_UNAVAILABLE");
...
await check("real WebSocket uncertain action joins after disconnect and recovery reaches completionRev", async () => {
  ...
  const wsA = new WebSocket(url); await opened(wsA);
  const clientA = hson.liveHost.client({ socket: ws_socket(wsA), clientId: "real-browser", recovery: { logicalMapId: f.host.stream.logicalMapId } });
  clientA.connect(); await clientA.recovery.recover();
  ...
  const wsB = new WebSocket(url); await opened(wsB);
  const clientB = hson.liveHost.client({ socket: ws_socket(wsB), clientId: "real-browser", map: clientA.map, recovery: { logicalMapId: f.host.stream.logicalMapId, cursor: { incarnationId: clientA.recovery.incarnationId, lastAppliedRev: clientA.recovery.lastAppliedRev } } });
  clientB.connect();
  await clientB.recovery.recover();
  assert.equal((await clientB.action_status(request.requestId)).state, "pending");
  const joined = clientB.retry_action(request);
  assert.equal((await clientB.action_status(request.requestId)).state, "pending");
  assert.equal(f.host.actionRequests.debug().joinedPendingDuplicateCount, 1);
  f.gate.resolve();
  const outcome = await joined;
  assert.equal(outcome.delivery, "joined");
  assert.equal(f.executions(), 1);
  assert.ok(clientB.recovery.lastAppliedRev >= outcome.completionRev);
  assert.equal((await clientB.action_status(request.requestId)).state, "succeeded");
  const closeB = closed(wsB); wsB.close(); await closeB;
  await new Promise((resolve) => server.close(resolve));
});

process.stdout.write(`LiveHost action dedupe acceptance checks passed (${checks}).\n`);
