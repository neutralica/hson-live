...
check("portable document restore starts a fresh empty identity epoch", () => { const m = element(`<main/>`); const source = element(`<article @${Q1}/>`); m.restore(source.capture()); assert.deepEqual(state(m), { epoch: 1, issued: 0 }); });
