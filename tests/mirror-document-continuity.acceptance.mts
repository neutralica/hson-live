...
export const HSON_LIVE_TEST_METADATA = Object.freeze({
  id: "mirror.document-continuity",
  title: "Document Mirror continuity",
  category: "Mirror",
  runtime: "node-synthetic-dom",
  tags: Object.freeze(["document", "binding", "continuity", "lifecycle", "externally-discoverable"]),
});

const testEvents = create_test_event_emitter("mirror.document-continuity");
let checks = 0;
...
check("bound direct structural mutation is rejected before drift", () => {
  ...
  assert.throws(
    ...
    (cause) => cause instanceof DocumentMirrorError
      && cause.code === DOCUMENT_MIRROR_UNSUPPORTED_OPERATION_ERROR_CODE,
  );
