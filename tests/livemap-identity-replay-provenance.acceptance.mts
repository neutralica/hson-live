...
check("canonical transport shape remains version-neutral", () => { const m = map({ x: {} }); const stream = make_locus_canonical_stream(m, { logicalMapId: "unit-12p", incarnationId: "wire" }); set_livemap_projected_quid_candidate_source_for_tests(m, () => Q1); acquire_projected_identity(m, ["x"]); const wire = structuredClone(stream.history.replayAfter(0)?.[0]); const decoded = decode_locus_canonical_commit(wire); assert.deepEqual(decoded, wire); assert.equal(Reflect.get(wire as object, "formatVersion"), undefined); });
