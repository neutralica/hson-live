...
export const HSON_LIVE_TEST_METADATA = Object.freeze({
  ...
  title: "Document LiveMap observation and replay",
  category: "LiveMap",
  runtime: "node",
  tags: Object.freeze(["document", "observation", "replay"]),
});

const testEvents = create_test_event_emitter("livemap.document-replay");
let checks = 0;
...
check("ordered element graph commits replay atomically without echo", () => {
  const source = element(`<main @000000002 <p @000000003 "old"/>/>`);
  ...
  assert.equal(target.document.byQuid("000000003")?.$_tag, "p");
...
check("replace-attrs planning is detached and leaves authority state untouched", () => {
  const map = element(`<main id="before" @00000001f/>`);
...
check("replace-attrs replays one detached final-state bag on path and QUID targets", () => {
  const map = element(`<main id="before" title="old" @000000020/>`);
  const attrs = {
    title: "after",
    empty: "",
    hidden: false,
...
check("all four public bulk attrs methods replay through the one replace-attrs path", () => {
  const initial = `<main id="old" title="kept" @000000025/>`;
  ...
  assert.equal(target.document.byQuid("000000025")?.$_tag, "main");
  assert.equal(target.document.byQuid("000000025")?.$_meta?.["quid"], "000000025");
});
...
check("malformed and out-of-order graph replay leave state unchanged", () => {
  const target = element(`<main @000000005/>`);
  const before = target.capture();
  const malformed = {
    changed: true,
    prevRev: 0,
