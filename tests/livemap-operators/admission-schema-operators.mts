...
export const admission_schema_operators: readonly DeterministicLiveMapOperator[] = Object.freeze([
  ...
  atomic_rejection_operator("admission/reject-boxed-primitive", "reject a boxed primitive", "Boxed primitives are arbitrary objects, not primitive projected values.", "Candidate is new Number(1).", 'new Number(1)', (map) => map.set(["value"], new Number(1) as unknown as JsonValue)),
  ...
  atomic_rejection_operator("admission/reject-undefined-item", "reject explicit undefined array data", "Present undefined is not a projected value.", "Own index 0 stores undefined.", 'length=1 values=[undefined]', (map) => map.set(["value"], [undefined] as unknown as JsonValue)),
