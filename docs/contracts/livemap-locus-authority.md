...
// livemap-locus-authority.md

# Locus Authority Contract
## Status
This document defines the initial authority and synchronization model for Locus.
The model is deliberately narrow.
Locus is a transport-independent authority and session layer. WebSocket is an adapter, not the architecture.
## Authority model
...
Locus core behavior must be testable with an in-memory socket-like adapter.
...
The initial Locus contract does not provide:
