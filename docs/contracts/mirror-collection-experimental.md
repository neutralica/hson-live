# Experimental Collection Mirror

Mirror exposes one deliberately narrow collection surface:
...
Collection Mirror requires an application `key` selector. Keys are strings or numbers,
and duplicates fail with `COLLECTION_MIRROR_DUPLICATE_KEY` before predictable
LiveTree mutation.
...
raises `COLLECTION_MIRROR_SOURCE_REPLACEMENT_FAILED`; the prior source and valid
...
const reflector = hson.mirror.collection({
...
Collection Mirror supports keyed array collections only, requires a dedicated
