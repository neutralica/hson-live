...
data `JsonValue`; identity and revision remain ordinary envelope fields.
...
This is data state transport, not graph-native recovery. Parsing the Hson
...
A Locus session may survive transport loss, but session survival is not required for state recovery.
...
Hosted tests must not define a separate reconnect protocol once generic Locus recovery is available.
...
Replace hosted-test-specific initial/commit recovery behavior with the generic Locus recovery protocol.
...
5. Snapshot data state and revision describe the same authoritative instant.
...
* Raw graph mutation that bypasses commits is outside Locus synchronization guarantees.
