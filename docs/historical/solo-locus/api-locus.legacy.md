...
The legacy one-map bootstrap codec has a versioned, QUID-free state body:
...
The raw one-map capture function and Node HTTP bootstrap helper are no longer
exported. They have no session projection and cannot serve as framework-hosted
client egress. For projected multi-library hosting, use a session-bound
`locus.cut(sessionId)` and place its HTML and projected state in the
...
Fixed Libraries authorities expose `AuthorityProjectionSnapshot` through the
session cut. It contains only the session's permitted authority libraries and
system features, with logical-map binding and no generated node identity.
...
`hson-live/locus/node` contains the one-map Node socket adapter.
`hson-live/livehost/node` contains the Node application-host/runtime
