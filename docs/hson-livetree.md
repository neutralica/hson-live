...
In the broader architecture, LiveTree is the view/projection layer: Hson defines the graph representation, LiveMap defines validated mutation and revisioned state, Locus governs a fixed hosted library registry and synchronization, applications own meaning and topology, LiveHost hosts applications, and LiveTree turns graph state into interactive browser presentation.
 -->
