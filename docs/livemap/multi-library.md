...
There is no default Library on a multi-map and no public removal, replacement, or rename operation. `map.lib.add(...)` changes local topology; `locus.lib.add(...)` admits hosted authority topology through its durable gate and exposure policy. QUID allocation remains map-wide within the underlying authority, but raw QUIDs do not route mutation requests across Libraries and identities cannot be transferred between Libraries. `root` and `snap` are selected-Library operations.
...
Connect the transport, establish a semantic session, then recover. Connection loss does not destroy the composed map: client-local roots, handles, and Mirror resources remain usable while disconnected and through compatible session reattachment. Live projection expansion installs granted authority libraries into that same map. Retained authority replay and snapshot fallback retain their existing behavior for compatible topology; reconciling changed topology across a disconnect is deferred. Client-local state currently lasts for the browser runtime; reload or runtime death initializes it again from application declarations.
