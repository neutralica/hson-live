...
- LiveProject structural planning calls `get_node_by_quid()` directly.
...
- `liveproject.keyed.ts` registers renderer cleanup.
...
- Descendants: `wrap_in_tree()`, every `ContentManager` selection,
  ...
  helpers.
- LiveProject document projection: `bind_document_livetree()` captures a cloned
  map node and calls `create_livetree()`.
- LiveProject keyed projection: renderers return LiveTrees; records detach,
...
- graft: parse target -> pre-admit -> link host element -> project children ->
  create handle.
...
- Structural reconciliation stages a shadow graph, validates final QUID claims
...
- Keyed projection treats application keys as reconciliation keys and attaches
  renderer cleanup to each rendered tree’s QUID.

LiveProject therefore borrows LiveMap authority and a LiveTree runtime. Binding
...
| Ambient projection document creation | `project-live-tree.ts`, `graft.ts`, LiveProject structure | Resolve from the target owner document or runtime projection call. |
...
| active document-binding weak set | `liveproject.document.ts` | Enforces one binding per exact map object, independent of QUID value. |
...
content selection, append preflight, lifecycle cleanup, and LiveProject
structural reconciliation inherit or receive that runtime explicitly.
...
- LiveProject document bindings borrow both LiveMap authority and the LiveTree
