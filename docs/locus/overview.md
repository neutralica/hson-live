...
The server may hold private and public libraries. At session creation, Locus normalizes an authorized effective projection from exposure policy, the request, and the read authorizer. Runtime exposure defaults to private. Each grant is an exact set of names. An active session may explicitly expand its grant with `locus.sessions.updateProjection`; the ordinary authorizer runs again with current connection context. A policy change never silently widens an existing grant. Reducing one requires session revocation and a new session. Hosted cuts and fresh bootstrap use the session's current projection. Topology-aware recovery after disconnection remains future work.
