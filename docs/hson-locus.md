...
Locus orders hosted transitions, durably gates them where configured, records
...
const host = hson.locus.create<DocumentState, DocumentActions>({
  state: { documents: {} },
  actions: {
...
Policies may return a boolean or promise. False is `LOCUS_ACTION_FORBIDDEN`; throw/rejection is the distinct generic `LOCUS_ACTION_AUTHORIZATION_FAILED`. Fresh executions, joining attempts, and cached retries are reauthorized independently; decisions are never cached. Omission is implicit allow and is not access control. This is authorization, not authentication, roles, ACL persistence, subscription authorization, or action-enumeration concealment. It adds no wire fields. Hosted document actions pass through this boundary.
...
Locus now retains and publishes canonical commits keyed by LiveMap revision.
...
Locus depends on a small socket-like interface:

```ts
type LocusSocketLike = Readonly<{
...
## LiveMap, LiveTree, and Locus together
