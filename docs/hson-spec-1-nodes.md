...
*	HSON accepts all valid JSON values. Typed values (1 vs "1") are preserved via the use of <_hson_str> and <_hson_val> VSNs (see §4, below)
...
* <_hson_elem> tags can contain multiple nodes with the same key/`$_tag`.
* <_hson_elem> tags can contain array-like items that have no key/`$_tag`.
* <_hson_elem> is represented as an array in the node graph structure

#### <_hson_obj>
Object: Represents a JSON object 'cluster'.
* <_hson_obj> tags encodes key–value pairs as child nodes.
* <_hson_obj> tags must contain unique keys/`$_tag` values; duplicate keys are invalid and will throw
* <_hson_obj> is represented as a JS object in the node graph structure

<_hson_obj> and <_hson_elem> are required for disambiguation of HsonNode 'clusters'. Though the shapes of <_hson_elem> and <_hson_obj> structures are very similar, each have differences that cause fatal errors in the other (such as JSON objects' requirement for unique keys, whereas HTML elements may contain multiple 'button' tags).

When serializing HTML as JSON (or vice-versa), <_hson_obj> or <_hson_elem> tags (depending) will clutter the node structure visually. These tags are essential for the transformers to correctly type node clusters correctly across round-trip conversions.

HSON is a notation that can faithfully serialize either JSON or HTML without this structural node clutter.

#### <_hson_arr>
Array: Represents a JSON array.
*	must contain only <_hson_ii>-wrapped children.
*	Order of items is preserved by <_hson_ii> nodes' `$_meta.data-_index` properties.

#### <_hson_ii>
Index Item: Represents a single array item and carries the index number.
*	must appear only as a child of <_hson_arr>.
...
#### <_hson_str>
String: Represents a string literal. Only <<_hson_str>> tags may contain raw string primitives in their `$_content` property.

#### <_hson_val>
Value: Represents a non-string primitive literal (number, boolean, null). Only <_hson_val> tags may contain raw non-string data in their `$_content` property
...
*	To ensure consistency, HSON's transformers wrap content under operation in a <_hson_root> tag during processin. This tag is unwrapped on serialization or appending to other nodes and is usually not exposed publicly.


### Note:
<_hson_elem> tags, as well as `$_attrs` properties, will only be derived from HTML sources of data.
<_hson_obj> tags, <_hson_arr>/<_hson_ii> tags will only be derived from JSON data.

⸻

## Primitive Values

Primitive values may only appear wrapped inside <_hson_str> or <_hson_val> nodes. Primitives appearing in the `$_content` of other tags will cause an error in the HSON parser. HSON's transformer chain handles all creation of such nodes and they are effectively hidden from the user.
...
Attribute ordering is canonicalized on first parsing for consistency. Attribute data is never coerced into `$_tag` or `$_content` values; it is always mapped within the `$_attrs`/HsonAttrs property.

⸻

## Mixed Content

Especially When derived from native HTML, HsonNode `$_content` may contain a mix of HsonNodes and primitive values (typically strings) - these must always be contained within <_hson_str> or <_hson_val> tags, as noted above
...
{a: "1", b: "2", "no key", d: "4"  }
Permitting and preserving this mixed structure in <_hson_elem>, but not in <_hson_obj> - even when that HTML element is parsed into JSON - is one of the main reasons for the existence of these VSN tags and allows faithful representation of:
*	HTML mixed content
