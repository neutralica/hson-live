...
### Basic methods
- `.append(branch: LiveTree, index?: number): LiveTree` — appends branch children into this node (unwraps `_elem`, mirrors DOM).
- `.empty(): LiveTree` — removes all children (deep detach) but keeps this node.
- `.removeChildren(): number` — removes only direct *node* children; returns count.
- `.removeSelf(): number` — removes this node from DOM + HSON; returns `1` or `0`.

### Finder
- `.find(q: string | HsonQuery): LiveTree | undefined` — first matching descendant.
